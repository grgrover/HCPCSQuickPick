...
async function updateDMEPOS(records) {
  console.log('inupdateDMEPOS before connection')
...
const getLCodes = (res) => {
  db.query("select HCPCS, Description from dmepos where HCPCS >='L5000' and HCPCS < 'L9900'")
    .then((data) => {
      res.send(data.rows)
    })
...
const getPrices = (codes, billingZone, res) => {
  var text = `select ${billingZone}, hcpcs, description from dmepos where `
  codes.forEach(code => {
    text += `hcpcs = '${code}' or `
  })
